...
def softmax_output_dice_class(output, target):
    eps = 1e-8
    o1 = (output == 1).float()
    t1 = (target == 1).float()
...
def test_dice_hd95_softmax(
    ...
    if dataname in ['BRATS2021', 'BRATS2020', 'BRATS2018', 'ucsf_pdgm']:
        num_cls = 4
    ...
    for i, data in enumerate(test_loader):
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    ...
                    pred_part, class_output = model(x_input, mask)
                    pred[:, :, h:h + patch_size, w:w + patch_size, z:z + patch_size] += pred_part
                    pred_class_list.append(class_output)
        pred = pred / weight
        b = time.time()
        pred = pred[:, :, :H, :W, :Z]
        pred = torch.argmax(pred, dim=1)
        probs = torch.softmax(torch.mean(torch.stack(pred_class_list), dim=0), dim=1)
        ...
        if dataname in ['BRATS2021', 'BRATS2020', 'BRATS2018', 'ucsf_pdgm']:
            scores_separate, scores_evaluation = softmax_output_dice_class(pred, target)

        for k, name in enumerate(names):
            msg = 'Subject {}/{}, {}/{}'.format((i + 1), len(test_loader), (k + 1), len(names))
            msg += '{:>20}, '.format(name)
            msg += '{:d}, '.format(pred_class.item())
            msg += '{:d}, '.format(target_class.item())
            msg += '{:.2f},{:.2f} '.format(probs[0, 0].item(), probs[0, 1].item())
            ...
            csv_writer.writerow(
                [name, pred_class.item(), target_class.item(), probs[0, 0].item(), probs[0, 1].item(), scores_evaluation[k][0], scores_evaluation[k][1], scores_evaluation[k][2], scores_evaluation[k][3]])
            file.close()
            logging.info(msg)
    ...
    msg += ', Classification:'
    msg += ' Accuracy: {:.4f}, Precision: {:.4f}, Recall: {:.4f}, F1-Score: {:.4f}'.format(
        accuracy, precision, recall, f1)
    print(msg)
    print(cm)
    logging.info(msg)
    model.train()
    return vals_dice_evaluation.avg, accuracy, precision, recall, f1
