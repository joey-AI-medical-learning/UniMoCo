...
def test_dice_hd95_softmax(
    ...
    one_tensor = torch.ones(1, patch_size, patch_size, patch_size).float().to(device)  # [1, 80, 80, 80]
    ...
    for i, data in enumerate(test_loader):
        ...
        for k, name in enumerate(names):
            ...
            csv_writer.writerow(
                [name, scores_evaluation[k][0], scores_evaluation[k][1], scores_evaluation[k][2], scores_evaluation[k][3]])
            file.close()
            logging.info(msg)
    msg = 'Average scores:'
    msg += ' DSC: '
    ...
    logging.info(msg)
    model.train()
    return vals_dice_evaluation.avg
