...
class Decoder_sep(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        return pred,(de_x1_out, de_x2_out, de_x3_out, de_x4_out)
...
class Decoder_fuse(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        return pred, (self.up2(pred1), self.up4(pred2), self.up8(pred3), self.up16(pred4)), (de_x1_out, de_x2_out, de_x3_out, de_x4_out)
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.kl_conv1 = nn.Conv3d(32, 8, kernel_size=1, stride=1, padding=0)
        self.kl_conv2 = nn.Conv3d(64, 16, kernel_size=1, stride=1, padding=0)
        self.kl_conv3 = nn.Conv3d(128, 32, kernel_size=1, stride=1, padding=0)
        self.kl_conv4 = nn.Conv3d(256, 64, kernel_size=1, stride=1, padding=0)
        ...
        self.flair_conv = nn.Conv3d(basic_dims * 16, transformer_basic_dims, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, mask):
        ...
        flair_intra_x5 = flair_intra_x5 + flair_x5_local
        ...
        if self.is_training:
            ...
            t1ce_pred, kl_t1ce_spe = self.decoder_sep(t1ce_x1, t1ce_x2, t1ce_x3, t1ce_x4, t1ce_x5)
            t1_pred, kl_t1_spe = self.decoder_sep(t1_x1, t1_x2, t1_x3, t1_x4, t1_x5)
            t2_pred, kl_t2_spe = self.decoder_sep(t2_x1, t2_x2, t2_x3, t2_x4, t2_x5)
            out_1 = torch.cat((kl_flair_spe[0], kl_t1ce_spe[0], kl_t1_spe[0], kl_t2_spe[0]),dim=1)
            out_2 = torch.cat((kl_flair_spe[1], kl_t1ce_spe[1], kl_t1_spe[1], kl_t2_spe[1]), dim=1)
            out_3 = torch.cat((kl_flair_spe[2], kl_t1ce_spe[2], kl_t1_spe[2], kl_t2_spe[2]), dim=1)
            out_4 = torch.cat((kl_flair_spe[3], kl_t1ce_spe[3], kl_t1_spe[3], kl_t2_spe[3]), dim=1)
            out_1_lk = self.kl_conv1(out_1)
            out_2_lk = self.kl_conv2(out_2)
            out_3_lk = self.kl_conv3(out_3)
            out_4_lk = self.kl_conv4(out_4)
            spe_out_kl = (out_1_lk, out_2_lk, out_3_lk, out_4_lk)
        ...
        x5_intra_local = self.multi_conv(torch.cat((flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5),dim=1))
        ...
        if self.is_training:
            return fuse_pred, (flair_pred, t1ce_pred, t1_pred, t2_pred), preds, spe_out_kl, kl_pred_fuse
        return fuse_pred
...
if __name__ == "__main__":
    ...
    mask = torch.from_numpy(np.array([[True, False, False, False], [ False, True, False, False]])).detach()
    y1 = model(x, mask)