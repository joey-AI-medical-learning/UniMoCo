...
from dataset.dataset_nii import BraTS_load_all_train_nii_segmentation, BraTS_load_all_test_nii_segmentation
...
parser.add_argument('--model', default='D2L-Net', type=str)
...
def main():
    ...
    train_set = BraTS_load_all_train_nii_segmentation(transforms=args.train_transforms, root=args.datapath, num_cls=num_cls,
                                         train_file=train_file)
    test_set = BraTS_load_all_test_nii_segmentation(transforms=args.test_transforms, root=args.datapath, test_file=test_file)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            ...
            fuse_pred, sep_preds, prm_preds, spe_out_kl, kl_pred_fuse = model(x, mask)
            ...
            fuse_loss = 0.5 *(fuse_cross_loss + fuse_dice_loss)
            ...
            sep_loss = 0.5 * (sep_cross_loss + sep_dice_loss)
            ...
            prm_loss = 0.5 * (prm_cross_loss + prm_dice_loss)

            mse_loss = torch.zeros(1).to(device).float()
            for sep_pre in sep_preds:
                mse_loss += F.mse_loss(sep_pre, fuse_pred)

            kl_loss = torch.zeros(1).to(device).float()
            weight_prm = 1.0
            for kl_pred, kl_spe in zip(kl_pred_fuse, spe_out_kl):
                weight_prm /= 2.0
                kl_loss += weight_prm * criterions.temp_kl_loss_bs(kl_pred, kl_spe)

            if epoch < args.region_fusion_start_epoch:
                loss = fuse_loss * 0.0 + sep_loss + prm_loss + mse_loss + kl_loss
            else:
                loss = fuse_loss + sep_loss + prm_loss + mse_loss + kl_loss
            ...
            writer.add_scalar('kl_loss', mse_loss.item(), global_step=step)
